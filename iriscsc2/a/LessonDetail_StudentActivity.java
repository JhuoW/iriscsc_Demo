...
import com.xzmc.airuishi.fragment.Student_StudentFragment;
import com.xzmc.airuishi.view.HeaderLayout;

public class LessonDetail_StudentActivity extends BaseActivity {
	...
	private void initView() {
		...
		headerLayout.showLeftBackButton(new OnClickListener() {
			...
			public void onClick(View v) {
				// TODO Auto-generated method stub
				LessonDetail_StudentActivity.this.finish();
			}
		});
		...
		fragments.add(new Student_StudentFragment());
	...
	private void changeState(int arg0) {
		if (arg0 == 0) {
			...
			ViewPropertyAnimator.animate(lessonDetail).scaleX(1.2f).setDuration(200);
			ViewPropertyAnimator.animate(lessonDetail).scaleY(1.2f).setDuration(200);
			ViewPropertyAnimator.animate(student).scaleX(1.0f)
					.setDuration(200);
			ViewPropertyAnimator.animate(student).scaleY(1.0f)
					.setDuration(200);

		} else {
			student.setTextColor(getResources().getColor(R.color.theme));
			lessonDetail.setTextColor(getResources().getColor(R.color.black));
			ViewPropertyAnimator.animate(lessonDetail).scaleX(1.0f).setDuration(200);
			ViewPropertyAnimator.animate(lessonDetail).scaleY(1.0f).setDuration(200);
			ViewPropertyAnimator.animate(student).scaleX(1.2f)
					.setDuration(200);
			ViewPropertyAnimator.animate(student).scaleY(1.2f)
					.setDuration(200);
		}
	}
}
